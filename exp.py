...
class Algorithm_exp():
    ...
    def nomatch(self, test, DIR):
        ...
        if self.NODELIST[self.state.row][self.state.column] in maru: # == "x":
            ...
            if self.M/(self.M+self.n) >= 0.5 + self.RATE:
                ...
                self.TRIGAR = True
                self.test.show(self.state, self.map, {}, DIR,     self.TRIGAR,     self.VIZL, self.VIZD)
    ...
    def Explore(self, STATE_HISTORY, state, TRIGAR, total_stress, grid, CrossRoad, x, TOTAL_STRESS_LIST, Backed_just_before, standard_list, rate_list, map_viz_test, DIR, VIZL, VIZD, LN, DN): # , PERMISSION):
        ...
        while not self.done:
                    ...
                    if rand > 1: # 0.8の確率で発見
                        ...
                        break # Advanceに移行する？
                            ...
                            if judge_node__x:
                                ...
                                print("=================")
                                self.threshold(pre)
                                self.test.show(self.state, self.map, {}, self.DIR,     self.TRIGAR,     self.VIZL, self.VIZD)
                                break
                else:
            ...
            self.action, self.bp_end, self.All_explore, self.TRIGAR, self.Reverse, self.lost = self.agent.mdp_exp(self.state, self.TRIGAR,     states_known, self.map, self.grid, DIR,     self.VIZL, self.VIZD)
        ...
        return self.total_stress, self.STATE_HISTORY, self.state, self.TRIGAR, self.CrossRoad, GOAL, self.TOTAL_STRESS_LIST, self.move_step, self.old_to_advance, self.phi, self.standard_list, self.rate_list, self.test_s, self.map, self.pre_action, self.VIZL, self.VIZD, self.L_NUM, self.D_NUM, self.exp_find